...
🟢 supabase - Ready (29 tools)
  ...
  - search_docs
    ...
    searchDocs(query: String!, limit: Int): SearchResultCollection
      ...
      """Get error codes that can be potentially returned by Supabase services"""
      errors(
...
In addition to the tables for the PT calculator, the following tables have been added to support the contextual help system:

### `pay_help_details`

This table stores the segmented help content for the Pay Calculator.

-   `title`: The primary key for lookup, corresponding to the `contentKey`.
-   `purpose_description`: The text for the "Purpose" section.
-   `recipient_group`: The text for the "Recipients" section (re-purposed for general details).
-   `report_section`: The text for the "Report Section" (re-purposed for general details).

### `retirement_help_details`

This table stores the segmented help content for the Retirement Calculator.

-   `title`: The primary key for lookup.
-   `purpose_description`: The text for the "Purpose" section.
