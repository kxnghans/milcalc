...
### 4.1 Fitness Standards (2025 Architecture)
The 2025 PT Architecture prioritizes **human auditability**. Performance values (e.g., `"13:25"`, `"<= 0.49"`, `"45-48"`) are stored as **Text** in the database to precisely match source DAFMAN 36-2905 PDFs.

-   **`pt_scoring_standards`**: Unified lookup for all exercise points. Includes `gender` and `age_group` columns for direct lookup.
...
-   **Parser Intelligence**: The `@repo/utils` logic engine features a `parsePerformanceRange` utility that handles ranges (`"45-48"`), inequalities (`">= 50"`), and exact values.
-   **Relational Groups**: Exercises are grouped via the `pt_groups` table, allowing the UI to dynamically build component lists (e.g., "Cardio", "Strength") based on backend configuration.
