...
export default function CardioComponent({
  ...
  const [adjustedWalkMaxTime, setAdjustedWalkMaxTime] = React.useState<
    number | null
  >(null);
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      if (cardioComponent === "run" && altitudeData.run) {
        ...
        const adjustmentRow = altitudeData.run.find(
          (row) =>
        ...
        const adjustmentRow = altitudeData.hamr.find(
          (row) =>
    ...
    <View>
      <View style={styles.exerciseBlock}>
        <View style={styles.componentHeader}>
          <TouchableOpacity
            onPress={() => openDetailModal(cardioComponent, getMascot())}
          >
            <Icon
              name={ICONS.HELP}
              size={16}
          ...
          {showProgressBars &&
            (() => {
              if (cardioComponent === "run") {
                const timeInSeconds =
                  (parseInt(runMinutes) || 0) * 60 +
                  (parseInt(runSeconds) || 0);
                return (
                  <View style={styles.progressBarContainer}>
                    <NeumorphicOutset>
                      <ProgressBar
                        ...
                        valueIsTime={true}
                        score={
                          typeof score.cardioScore === "number"
                            ? score.cardioScore
                            : 0
                        }
                        maxScore={50}
                ...
                const timeInSeconds =
                  (parseInt(walkMinutes) || 0) * 60 +
                  (parseInt(walkSeconds) || 0);
                const passThreshold =
                  adjustedWalkMaxTime !== null
                    ? adjustedWalkMaxTime
                    : cardioMinMax.min;
                return (
                  <View style={styles.progressBarContainer}>
        ...
        <SegmentedSelector
          options={[
            { label: "2-Mile Run", value: "run" },
            { label: "20m HAMR", value: "shuttles" },
            { label: "2-km Walk", value: "walk" },
          ]}
