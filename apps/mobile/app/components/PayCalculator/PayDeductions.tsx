...
interface PayDeductionsProps {
  ...
  setAdditionalDeductions: React.Dispatch<React.SetStateAction<AdditionalDeduction[]>>;
      ...
      <View style={styles.fieldRow}>
          ...
          {isDeductionsExpanded && (
              <View style={[styles.expandableContent, { marginTop: 0 }]}>
                  ...
                  {isTaxOverride && (
                      <>
                          <View style={styles.fieldRow}><NewLabelWithHelp label="Federal Tax" contentKey="Federal Tax" onHelpPress={openDetailModal} /><CurrencyInput placeholder="0.00" value={deductions.overrideFedTax} onChangeText={(text) => setDeductions(d => ({...d, overrideFedTax: text}))} /></View>
                          <View style={styles.fieldRow}><NewLabelWithHelp label="State Tax" contentKey="State Tax" onHelpPress={openDetailModal} /><CurrencyInput placeholder="0.00" value={deductions.overrideStateTax} onChangeText={(text) => setDeductions(d => ({...d, overrideStateTax: text}))} /></View>
                          <View style={styles.fieldRow}><NewLabelWithHelp label="FICA" contentKey="FICA" onHelpPress={openDetailModal} /><CurrencyInput placeholder="0.00" value={deductions.overrideFicaTax} onChangeText={(text) => setDeductions(d => ({...d, overrideFicaTax: text}))} /></View>
                      </>
                  )}

                  <View style={styles.fieldRow}><NewLabelWithHelp label="SGLI" contentKey="Servicemembers' Group Life Insurance (SGLI)" onHelpPress={openDetailModal} /><CurrencyInput placeholder="0.00" value={deductions.sgli} onChangeText={(text) => setDeductions(d => ({...d, sgli: text}))} /></View>
                  <View style={styles.fieldRow}><NewLabelWithHelp label="TSP" contentKey="Thrift Savings Plan (TSP)" onHelpPress={openDetailModal} /><CurrencyInput placeholder="0.00" value={deductions.tsp} onChangeText={(text) => setDeductions(d => ({...d, tsp: text}))} /></View>
