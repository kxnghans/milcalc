  ...
  useEffect(() => {
    const checkHistory = async () => {
      try {
        ...
        interface SplashHistory {
          launchedEver: boolean;
          lastShownSeason: string;
        }

        const history: SplashHistory = result
          ? (JSON.parse(result.value) as SplashHistory)
          : { launchedEver: false, lastShownSeason: "" };
        const currentSeason = getSeason();
        const today = new Date();

        // Season starts on the 1st of month 2 (March), 5 (June), 8 (September), 11 (December)
        const isFirstDayOfSeason =
          today.getDate() === 1 && [2, 5, 8, 11].includes(today.getMonth());
        ...
        if (!history.launchedEver) {
          ...
          isFirstDayOfSeason &&
          history.lastShownSeason !== currentSeason
        ) {
          shouldShow = true;
        }
