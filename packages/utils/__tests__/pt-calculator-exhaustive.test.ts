...
import { getScoreForExercise, calculatePtScore } from '../src/pt-calculator';
...
describe('PT Calculator - Exhaustive CSV Validation', () => {
    ...
    const exercises = Array.from(new Set(exhaustiveExpectedScores.map(s => s.exercise)));

    exercises.forEach(exercise => {
        describe(`Exercise: ${exercise}`, () => {
            const exerciseScores = exhaustiveExpectedScores.filter(s => s.exercise === exercise);
            
            // Further group by gender and age group for better structure
            const genderAges = Array.from(new Set(exerciseScores.map(s => `${s.gender}|${s.ageGroup}`)));

            genderAges.forEach(ga => {
                const [gender, ageGroup] = ga.split('|');
                const groupScores = exerciseScores.filter(s => s.gender === gender && s.ageGroup === ageGroup);
                ...
                it(`should match all CSV points for ${gender} ${ageGroup}`, () => {
                    ...
                    uniqueTests.forEach(expected => {
                        ...
                        if (exercise === 'whtr') {
                            ...
                            resultPoints = typeof res.whtrScore === 'number' ? res.whtrScore : 0;
                        } else {
                            ...
                            if (exercise === 'run' || exercise === 'run_2mile') {
                                ...
                                performance = { minutes: parts[0], seconds: parts[1] || 0 };
                                 ...
                                 const parts = cleanPerf.split(':').map(Number);
                                 performance = { minutes: parts[0], seconds: parts[1] || 0 };
                            } else if (exercise === 'shuttles' || exercise === 'shuttles_20m') {
                            ...
                            const res = getScoreForExercise(standards, exercise, performance, undefined, undefined, expected.performance);
                        ...
                        try {
                            ...
                            throw new Error(`Failed for ${gender} ${ageGroup} ${exercise} perf="${expected.performance}": Expected ${expected.points}, Got ${resultPoints}`);
                        }
                    });
                });
            });
