import { checkWalkPass, getScoreForExercise } from "../../src/pt-calculator";
...
describe("PT Calculator - Altitude Adjustments", () => {
  ...
  const mockPassFail: Tables<"pt_pass_fail_standards">[] = [
    {
      ...
      gender: "Male",
      age_group: "25-29",
      min_performance: "18:00",
      max_performance: null,
      created_at: "",
    },
  ];
  ...
  describe("2-Mile Run Altitude Adjustment", () => {
    it("should subtract 15s from run time for Group 1 (0-14:00)", () => {
      ...
      const res = getScoreForExercise(
        mockStandards,
        "run_2mile",
        { minutes: 14, seconds: 10 },
        "Group 1",
        mockAltitudeCorrections,
      );
  ...
  describe("2-km Walk Altitude Adjustment", () => {
    it("should use altitude max_time instead of normal min_performance", () => {
      ...
      const normal = checkWalkPass(
        25,
        "Male",
        19,
        30,
        mockPassFail,
        mockWalkAltThresholds,
        "normal",
      );
      expect(normal).toBe("fail");

      const altitude = checkWalkPass(
        25,
        "Male",
        19,
        30,
