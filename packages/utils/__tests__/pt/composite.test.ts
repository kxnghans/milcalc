...
describe("Dynamic Complete Matrix Validation", () => {
  ...
  const combos = genders.flatMap((g) =>
    ageGroups.map((a) => ({ gender: g, ageGroup: a })),
  );

  test.each(combos)(
    "Standards load and score for $gender $ageGroup",
    ({ gender, ageGroup }) => {
