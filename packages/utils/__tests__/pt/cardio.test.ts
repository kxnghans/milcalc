...
describe('HAMR Range Scoring', () => {
    ...
    it('Male <25: 84 shuttles = 49.5 pts', () => {
        const r = getScoreForExercise(std('Male', '<25'), 'shuttles_20m', { shuttles: 84 });
    ...
    it('Male <25: 67 shuttles = 44.0 pts', () => {
        const r = getScoreForExercise(std('Male', '<25'), 'shuttles_20m', { shuttles: 67 });
        expect(r.points).toBeCloseTo(44.0, 1);
    });
    it('Male 55-59: >= 69 shuttles = 50 pts', () => {
        const r = getScoreForExercise(std('Male', '55-59'), 'shuttles_20m', { shuttles: 69 });
        expect(r.points).toBe(50);
    });
    it('Female <25: >= 68 shuttles = 50 pts', () => {
        const r = getScoreForExercise(std('Female', '<25'), 'shuttles_20m', { shuttles: 68 });
        expect(r.points).toBe(50);
    });
...
describe('Walk Test (2km)', () => {
    it('Female 38: walk 17:20 → pass (under standard 17:28)', () => {
        const r = score('Female', '35-39', {
            ...
            cardioComponent: 'walk', walkMinutes: 17, walkSeconds: 20,
            whtr: 0.49,
        });
    ...
    it('Female 38: walk 17:30 → fail (over standard 17:28)', () => {
        const r = score('Female', '35-39', {
            pushupComponent: 'push_ups_1min', pushups: 42,
            coreComponent: 'sit_ups_1min', situps: 43,
            cardioComponent: 'walk', walkMinutes: 17, walkSeconds: 30,
            whtr: 0.49,
        });
