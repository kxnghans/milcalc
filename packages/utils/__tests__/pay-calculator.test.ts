...
import {
  ...
  calculateNetPayWithDisability,
  calculatePay,
} from "../src/pay-calculator";
import { Tables } from "../src/types";
import {
  mockDisabilityData,
  mockFederalTaxData,
  mockStateTaxData,
} from "./test-mocks/pay-data-mocks";
...
describe("Pay Calculator", () => {
  ...
  describe("calculatePay", () => {
    it("should calculate pay correctly for a single individual in CA", () => {
      ...
      const result = calculatePay(
        inputs,
        mockFederalTaxData as Tables<"federal_tax_data">[],
        mockStateTaxData as Tables<"state_tax_data">[],
      );

      expect(result.totalIncome).toBeCloseTo(7700);
      expect(result.ficaTax).toBeCloseTo(382.5);
  ...
  describe("calculateDisabilityIncome", () => {
    it("should return the correct disability income for a veteran alone", () => {
      const income = calculateDisabilityIncome(
        "70%",
        "Veteran alone",
        mockDisabilityData as Tables<"veterans_disability_compensation">[],
      );
