...
function processInvertedScoringCSV(
  csvFile: string,
  exerciseType: string,
): string[] {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    for (const age of AGE_GROUPS) {
      ...
      output.push(
        `${exerciseType},${gender},${age},${performance},${points.toFixed(1)},`,
      );
    }
  }
  return output;
...
function generateScoringStandards() {
  const scoringRows: string[] = [
    "exercise_type,gender,age_group,performance,points,health_risk_category",
  ];

  scoringRows.push(
    ...processInvertedScoringCSV(
      "pt_scoring_pushups_1min.csv",
      "push_ups_1min",
    ),
  );
  scoringRows.push(
    ...processInvertedScoringCSV("pt_scoring_situps_1min.csv", "sit_ups_1min"),
  );
  scoringRows.push(
    ...processInvertedScoringCSV(
      "pt_scoring_hand_release_pushups_2min.csv",
      "hand_release_pushups_2min",
    ),
  );
  scoringRows.push(
    ...processInvertedScoringCSV(
      "pt_scoring_cross_leg_reverse_crunch_2min.csv",
      "cross_leg_reverse_crunch_2min",
    ),
  );
  scoringRows.push(
    ...processInvertedScoringCSV(
      "pt_scoring_forearm_plank.csv",
      "forearm_plank_time",
    ),
  );
  scoringRows.push(
    ...processInvertedScoringCSV("pt_scoring_run_2mile.csv", "run_2mile"),
  );
  scoringRows.push(
    ...processInvertedScoringCSV(
      "pt_scoring_hamr_shuttles.csv",
      "shuttles_20m",
    ),
  );
  ...
  for (let i = 1; i < whtrRows.length; i++) {
    ...
    scoringRows.push(
      `whtr,Both,All,${performance},${points.toFixed(1)},${risk}`,
    );
  }

  fs.writeFileSync(
    path.join(uploadDir, "pt_scoring_standards.csv"),
    scoringRows.join("\n"),
  );
...
function generatePassFailStandards() {
  ...
  const passFailRows: string[] = [
    "exercise_type,gender,age_group,min_performance",
  ];
  for (let i = 1; i < rows.length; i++) {
    const row = rows[i];
  ...
  fs.writeFileSync(
    path.join(uploadDir, "pt_pass_fail_standards.csv"),
    passFailRows.join("\n"),
  );
...
function generateAltitudeCorrections() {
  const correctionsRows: string[] = [
    "exercise_type,altitude_group,altitude_range,perf_start,perf_end,correction",
  ];
  ...
  for (let i = 1; i < runRows.length; i++) {
    ...
    correctionsRows.push(
      `run_2mile,${row[0]},${row[1]},${row[2]},${row[3]},${row[4]}`,
    );
  }
  ...
  fs.writeFileSync(
    path.join(uploadDir, "pt_altitude_corrections.csv"),
    correctionsRows.join("\n"),
  );
...
function generateAltitudeWalkThresholds() {
  const walkRows: string[] = [
    "sex,altitude_group,altitude_range,age_range,max_time",
  ];
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    walkRows.push(
      `${row[0]},${row[1]},${row[2]},${row[3]}-${row[4]},${max_time}`,
    );
  }

  fs.writeFileSync(
    path.join(uploadDir, "pt_altitude_walk_thresholds.csv"),
    walkRows.join("\n"),
  );
