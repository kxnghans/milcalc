...
function generateScoringSeed() {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    values.push(
      `  ('${exercise}', '${gender}', '${age}', '${performance}', ${points}, ${riskVal})`,
    );
  }
...
function generatePassFailSeed() {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    values.push(
      `  ('${exercise}', '${gender}', '${age}', '${minPerf}', ${maxPerfSql})`,
    );
  }
...
function generateAltitudeSeed() {
  ...
  for (let i = 1; i < corrRows.length; i++) {
    ...
    corrValues.push(
      `  ('${exercise}', '${group}', '${range}', ${startVal}, ${endVal}, ${corr})`,
    );
  }
  sql += corrValues.join(",\n");
  sql += `;\n\n`;
