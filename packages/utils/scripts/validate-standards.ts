...
function validateScoringSeed(
  ...
  for (let i = 1; i < csvRows.length; i++) {
    ...
    for (const age of ageGroups) {
      ...
      if (isNaN(expectedPoints) || !performance || performance === "N/A")
        continue;
      ...
      const sqlPattern = new RegExp(
        `'${exerciseType}',\\s*'${gender}',\\s*'${escapedAge}',\\s*'${escapedPerf}',\\s*${pointsPattern}`,
        "i",
      );

      if (!sqlPattern.test(sqlContent)) {
        console.error(
          `  [MISSING] ${gender} ${age} perf=${performance} points=${expectedPoints}`,
        );
        errors++;
      }
    }
...
function validateWhtr() {
  ...
  for (let i = 1; i < csvRows.length; i++) {
    ...
    const escapedPerf = performance.replace(/[.*+?^${}()|[\]\\]/g, "\\$&");
    const sqlPattern = new RegExp(
      `'whtr',\\s*'${escapedPerf}',\\s*${points.toFixed(1)}`,
      "i",
    );
    if (!sqlPattern.test(sqlContent)) {
...
function validateAll() {
  ...
  totalErrors += validateScoringSeed(
