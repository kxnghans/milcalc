 ...
 */
export const getPtStandards = async (
  gender: string,
  age_group: string,
): Promise<PtStandard[] | null> => {
 ...
 */
export const getPassFailStandards = async (
  gender: string,
  age_group: string,
): Promise<Tables<"pt_pass_fail_standards">[] | null> => {
  const { data, error } = await supabase
 ...
 */
export const getPtAltitudeCorrections = async (): Promise<
  Tables<"pt_altitude_corrections">[] | null
> => {
  const { data, error } = await supabase
 ...
 */
export const getPtAltitudeWalkThresholds = async (
  gender: string,
): Promise<Tables<"pt_altitude_walk_thresholds">[] | null> => {
  const { data, error } = await supabase
 ...
 */
export const getAltitudeAdjustments = async (
  exercise: "run" | "walk" | "hamr",
): Promise<Tables<"pt_altitude_corrections">[] | null> => {
...
export const getHelpContent = async (contentKey: string) => {
  ...
  const combinedContent = data.reduce(
    (acc: { [key: string]: string | null }, item) => {
 ...
 */
export const getPtStandardsBundle = async (
  gender: string,
  age_group: string,
): Promise<{
