...
export const StyledButton = ({
  ...
  const Icon = Icons[iconSet] as React.ElementType;
  ...
  const finalIconSize =
    iconSize ??
    (size === "small"
      ? theme.typography.subtitle.fontSize + 2
  ...
  const radiusStyles = {
    borderRadius:
      borderRadius !== undefined ? borderRadius : theme.borderRadius.m,
    borderTopLeftRadius,
    borderTopRightRadius,
    borderBottomLeftRadius,
    borderBottomRightRadius,
  ...
  return (
    ...
    <NeumorphicOutset
      ...
      highlightOpacity={
        isPrimary ? (isDarkMode ? 0.3 : 1) : isDarkMode ? 0.01 : 1
      }
    >
      <TouchableOpacity style={[styles.button, buttonSizeStyle]} {...props}>
        ...
        <Text style={[styles.text, variantTextStyle, textSizeStyle, textStyle]}>
          {title}
        </Text>
      </TouchableOpacity>
    </NeumorphicOutset>
