...
function usePayApiData(
  ...
  const { data: bah, isLoading: isLoadingBah } = useQuery({
    ...
    queryFn: () =>
      getBahRate(
        debouncedMha,
        debouncedRank as string,
        debouncedBahDependencyStatus as
          | "WITH_DEPENDENTS"
          | "WITHOUT_DEPENDENTS",
      ),
    enabled:
      !!debouncedMha &&
      debouncedMha !== "initial" &&
      debouncedMha !== "ON_BASE" &&
      !!debouncedRank &&
      !!debouncedBahDependencyStatus,
  });
...
export const usePayCalculatorState = (
  ...
  const totalDeductions = useMemo(() => {
    ...
    if (isTaxOverride) {
      taxesTotal =
        parseCurrency(debouncedDeductions.overrideFedTax) +
        parseCurrency(debouncedDeductions.overrideStateTax) +
        parseCurrency(debouncedDeductions.overrideFicaTax);
    } else {
      taxesTotal =
        calculatedTaxes.fedTax +
        calculatedTaxes.stateTax +
        calculatedTaxes.ficaTax;
    }
  ...
  const deductionsForDisplay = useMemo(() => {
    const fedTaxValue = isTaxOverride
      ? parseCurrency(debouncedDeductions.overrideFedTax)
      : calculatedTaxes.fedTax;
    const stateTaxValue = isTaxOverride
      ? parseCurrency(debouncedDeductions.overrideStateTax)
      : calculatedTaxes.stateTax;
    const ficaTaxValue = isTaxOverride
      ? parseCurrency(debouncedDeductions.overrideFicaTax)
      : calculatedTaxes.ficaTax;
  ...
  const showAddIncomeButton = !!(
    lastAdditionalIncome &&
    lastAdditionalIncome.name &&
    lastAdditionalIncome.amount
  );

  const lastAdditionalDeduction =
    additionalDeductions[additionalDeductions.length - 1];
  const showAddDeductionButton = !!(
    lastAdditionalDeduction &&
    lastAdditionalDeduction.name &&
    lastAdditionalDeduction.amount
  );
